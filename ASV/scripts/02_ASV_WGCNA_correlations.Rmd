...
samples_merged <- inner_join(asv_samples, sample.description.timecourse.live, by = "sampleID") %>%
  ...
  filter(!duplicated(RNA_ID)) # due to a problem with ASV sample description. hopefully can be resolved.
samples_merged
```


```{r}
MEs <- MEs[samples_merged$RNA_ID,]
head(MEs[,1:10])
...
```{r}
all(rownames(MEs)==samples_merged$RNA_ID) # double check!
rownames(MEs) <- samples_merged$ASV_ID
```
...
MEcors <- MEs %>%
  ...
  pivot_longer(-sampleID, names_to = "module", values_to = "expression",cols_vary = "slowest") %>%
  nest(.by = module)

#confirm order is still okay

all(MEcors$data[[1]]$sampleID == rownames(asv_counts_small))

system.time(MEcors <- MEcors %>%
              mutate(pearson.r = map(data, ~ cor(.x$expression, asv_counts_small)),
                     pearson.perm = map(data, ~ permute.cor(.x$expression, asv_counts_small, method = "pearson")),
                     spearman.rho = map(data, ~ cor(.x$expression, asv_counts_small, method = "spearman")),
                     spearman.perm = map(data, ~ permute.cor(.x$expression, asv_counts_small, method = "spearman")),
                     kendall.tau = map(data, ~ cor(.x$expression, asv_counts_small, method = "kendall")),
                     kendall.perm = map(data, ~ permute.cor(.x$expression, asv_counts_small, method = "kendall")),
                     ))
...
```
```{r}
save(MEcors, asv_counts_small, taxa, samples_merged, file = "../output/ME_ASV_cors_live.Rdata")
```
