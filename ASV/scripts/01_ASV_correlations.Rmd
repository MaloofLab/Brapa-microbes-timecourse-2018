...
graphstibblepermute <- tibble(perm=1:nperms, cutoffs=list(cutoffs) ) %>%
  mutate(MR_live=map2(perm, cutoffs, doPerm, rna=rna_live, asv=asv_live_small))
...
```{r}
graphstibble %>% select(-graph,-nodes,-title, nNode, maxcor, meancor)
```
