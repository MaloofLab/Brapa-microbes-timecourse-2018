...
```{r}
graphstibble <- tibble(cutoff=c(1, 3, 5, 10) )
graphstibble
```
...
```{r}
pdf("../output/ASV_graphs.pdf",height=12,width=12)
graphstibble %>%
...
getcorMRperm <- function(perm, rna, asv) {
  print(perm)
  getcorMR(rna=apply(rna, 2, sample),
           asv=apply(asv, 2, sample))
}

graphstibblepermute <- tibble(perm=1:nperms, list(cutoffs) ) %>%
  mutate(MR_live=map(perm, getcorMRperm, rna=tmpRNA, asv=tmpASV))



# Make graphs and pull out subgraphs centered on ASVs that have an edge with a gene
graphstibblepermute <- graphstibblepermute %>%
  mutate(graph=map(cutoff, ~ get.mr.subgraph(
                      mr.cutoff=.x,
                      mr.matrix=MR_live$MR,
                      annotation = annotation,
...
graphstibblepermute <- graphstibblepermute %>%
  mutate(maxcor=map_dbl(graph, maxcor),
         meancor=map_dbl(graph, meancor),
         nNode=map_int(graph, ~length(V(.))))
graphstibblepermute %>% select(title, maxcor, meancor, nNode)
```
