...
JM: Want to come up with a cleaner way to create goSeq object
...
# Calculate FDR and add descriptions
go.results <- go.results %>%
  mutate(module.goseq = map(module.goseq,
                            ~ mutate(.x,
                                     over_represented_FDR = p.adjust(over_represented_pvalue,
                                                                     method = "fdr"),
                                     GO_description = ### TO DO ###
                                     )
                            )
         )

# Add in GO descriptions


Definition(GOTERM(go.results$module.goseq[[1]]$category))
```
...
# using unnest()
temp %>% unnest(GO_result) %>% dplyr::select(-transcripts) %>% write_csv(path="../output/twoafternoon.any.trtsoil.DEG.Kmeans.5cluster.csv")
# 6 Kmeans cluster
temp <- tibble(transcript_ID=cpm.timecourse.v3.0.scale.twoafternoon.any.trtlive.DEG.spread$transcript_ID, cluster=kClusters.any.trtlive.6) %>% 
  ...
  mutate(GO_result = map(transcripts, ~ GOseq.Brgo.v3.0.Atgoslim.BP.list.ORA(pull(.)))) 
# using unnest()
temp %>% unnest(GO_result) %>% dplyr::select(-transcripts)  %>% write_csv(path="../output/twoafternoon.any.trtsoil.DEG.Kmeans.6cluster.csv")
# 8 Kmeans cluster
...
# using unnest()
temp %>% unnest(GO_result) %>% dplyr::select(-transcripts) %>% write_csv(path="../output/twoafternoon.any.trtsoil.DEG.Kmeans.8cluster.csv") # I like this way to have target genes in one data.frame
# 15 Kmeans cluster
temp <- tibble(transcript_ID=cpm.timecourse.v3.0.scale.twoafternoon.any.trtlive.DEG.spread$transcript_ID, cluster=kClusters.any.trtlive.15) %>% 
  ...
  mutate(GO_result = map(transcripts, ~ GOseq.Brgo.v3.0.Atgoslim.BP.list.ORA(pull(.)))) 
# using unnest()
temp %>% unnest(GO_result) %>% dplyr::select(-transcripts)   %>% write_csv(path="../output/twoafternoon.any.trtsoil.DEG.Kmeans.15cluster.csv")
```
...
=======
dge.diurnal34.any.trtlive.DEGs.all.v3.0anno
>>>>>>> abb722b8016c541dc94e8c9827ca932c9c992d33
```
...
=======
dge.diurnal1314.any.trtlive.DEGs.all.v3.0anno
>>>>>>> abb722b8016c541dc94e8c9827ca932c9c992d33
```

```{r}
>>>>>>> cdd0e09e70ecaa35d1b1306e02567010c4d40578
# scaling expression data
...
```{r include=FALSE}
# 6 Kmeans cluster
temp <- tibble(transcript_ID=cpm.timecourse.v3.0.scale.diurnal34.any.trt.DEG.spread$transcript_ID, cluster=kClusters.diurnal34.any.trt.6) %>% 
  group_by(cluster) %>%
  nest(transcripts=transcript_ID) %>%
  mutate(GO_result = map(transcripts, ~ GOseq.Brgo.v3.0.Atgoslim.BP.list.ORA(pull(.)))) 
# using unnest()
temp %>% unnest(GO_result) %>% dplyr::select(-transcripts) %>% write_csv(path="../output/diurnal34.any.trt.DEG.Kmeans.6cluster.csv")
# 15 Kmeans cluster
temp <- tibble(transcript_ID=cpm.timecourse.v3.0.scale.diurnal34.any.trt.DEG.spread$transcript_ID, cluster=kClusters.diurnal34.any.trt.15) %>% 
  group_by(cluster) %>%
  nest(transcripts=transcript_ID) %>%
  mutate(GO_result = map(transcripts, ~ GOseq.Brgo.v3.0.Atgoslim.BP.list.ORA(pull(.)))) 
# using unnest()
temp %>% unnest(GO_result) %>% dplyr::select(-c(GO_result,transcripts))  %>% write_csv(path="../output/diurnal34.any.trt.DEG.Kmeans.15cluster.csv")
```

# GO ORA under construction (day 13/14)
```{r include=FALSE}
# 6 Kmeans cluster
temp <- tibble(transcript_ID=cpm.timecourse.v3.0.scale.diurnal1314.any.trt.DEG.spread$transcript_ID, cluster=kClusters.diurnal1314.any.trt.6) %>% 
  group_by(cluster) %>%
  nest(transcripts=transcript_ID) %>%
  mutate(GO_result = map(transcripts, ~ GOseq.Brgo.v3.0.Atgoslim.BP.list.ORA(pull(.)))) 
# using unnest()
temp %>% unnest(GO_result) %>% dplyr::select(-transcripts) %>% write_csv(path="../output/diurnal1314.any.trt.DEG.Kmeans.6cluster.csv")
# 15 Kmeans cluster
temp <- tibble(transcript_ID=cpm.timecourse.v3.0.scale.diurnal1314.any.trt.DEG.spread$transcript_ID, cluster=kClusters.diurnal1314.any.trt.15) %>% 
  group_by(cluster) %>%
  nest(transcripts=transcript_ID) %>%
  mutate(GO_result = map(transcripts, ~ GOseq.Brgo.v3.0.Atgoslim.BP.list.ORA(pull(.)))) 
# using unnest()
temp %>% unnest(GO_result) %>% dplyr::select(-transcripts) %>% write_csv(path="../output/diurnal1314.any.trt.DEG.Kmeans.15cluster.csv")
```
