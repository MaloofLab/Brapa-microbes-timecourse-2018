...
# write_cls2 <-function(x,y) { # x is dafa.frame y is file name
#   print("x is");print(x)
# Total <- dim(x)[1];print(Total)
# description <- "# LIVE BLANK";print(description)
# Live.Blank <- x %>% dplyr::select(matches("soil_trt")) %>% as_vector()
# Live.Blank <- ifelse(Live.Blank=="SBC_OLD",0,1) # "live" is zero 
# print(Live.Blank)
# # writing cls file
# cat(paste(Total,"2","1","\n"),file=file.path("..","output",paste(y,".csl",sep="")))
# cat(paste(description,"\n"),file=file.path("..","output",paste(y,".csl",sep="")),append=TRUE)
# cat(Live.Blank,file=file.path("..","output",paste(y,".csl",sep="")),append=TRUE)
# }
for(i in 1:5) {
...
GSEA_reports.DF.custom$sampling_time <- gsub("(1_morn|2_afternoon|3_evening_5.30|4_night_1|5_night_2)(\\.)(01|02|03|04|06|13|14)(.custom.Gsea.)([[:print:]]+)","\\1", GSEA_reports.DF.custom$report2)
GSEA_reports.DF.custom$sampling_day <- gsub("(1_morn|2_afternoon|3_evening_5.30|4_night_1|5_night_2)(\\.)(01|02|03|04|06|13|14)(.custom.Gsea.)([[:print:]]+)","\\3", GSEA_reports.DF.custom$report2)
# Live or Blank
...
sampling_day_time_soil.summary <- dge$samples %>% dplyr::count(sampling_day,sampling_time,soil_trt) %>% mutate(LorB=ifelse(soil_trt=="ATM_BLANK","B","L")) %>% unite(time_day_LorB,sampling_time,sampling_day,LorB,remove=FALSE)

# "all"  
all <- tibble(sampling_day_time_soil = rep(sampling_day_time_soil.summary$time_day_LorB,each=length(genes.count2$NAME)),
              NAME=rep(genes.count2$NAME,dim(sampling_day_time_soil.summary)[1])) %>% unite(time_day_LorB_NAME,sampling_day_time_soil,NAME,remove=FALSE) 
```
...
GSEA_reports.DF.formatted <- data  %>%  dplyr::select(sampling_time, sampling_day,LorB,NAME,`FDR q-val`) %>% mutate(`FDR q-val`=as.numeric(str_replace(`FDR q-val`,"^0$","0.01"))) %>% mutate(`-log10 FDR q-val`=-log10(`FDR q-val`)) %>% dplyr::select(-`FDR q-val`) %>% unite(time_day_LorB,sampling_time,sampling_day, LorB,remove=TRUE) %>% arrange(NAME)
## check
...
GSEA_reports.DF.formatted <- GSEA_reports.DF.formatted %>% unite(time_day_LorB_NAME, time_day_LorB,NAME,remove=FALSE) 
missing <- sampling_day_time_soil.summary
 %>% anti_join(GSEA_reports.DF.formatted,by="time_day_LorB_NAME") %>% mutate(`-log10 FDR q-val`=0) %>% rename(time_day_LorB=sampling_day_time_soil)
...
# LorB
if(LorB=="L") {
  GSEA_reports.DF.formatted.all <- GSEA_reports.DF.formatted.all %>% filter(str_detect(time_day_LorB,"_L"))
} else {
    GSEA_reports.DF.formatted.all <- GSEA_reports.DF.formatted.all %>% filter(str_detect(time_day_LorB,"_B"))
    }
print("after LorB selection")
GSEA_reports.DF.formatted.all %>% View()
# plot (base)
GSEA_reports.DF.plot <- GSEA_reports.DF.formatted.all %>% 
  ggplot(aes(x=NAME,y=time_day_LorB)) + geom_tile(aes(fill=`-log10 FDR q-val`),colour="black")  + scale_fill_gradient2(limit=c(0,3), high=muted("magenta")) # + facet_grid(~tissue) # not OK
...
# combined
GSEA.timecourse_soil_all <- plot_grid(GSEA.timecourse_soil_LIVE,GSEA.timecourse_soil_BLANK,align="h",labels=c("Live","Blank"))
ggsave2(GSEA.timecourse_soil_all,filename="GSEA.timecourse_soil_all.png",path=file.path("..","output"),width=20,height=8)
