...
GSEA_reports.DF.plot <- GSEA_reports.DF.formatted.all %>% 
  ggplot(aes(x=category3.tissue.au.up_down,y=time_day_LorB)) + geom_tile(aes(fill=`-log10 FDR q-val`),colour="black")  + scale_fill_gradient2(limit=c(0,3), high=muted("magenta")) 

# + scale_x_discrete(sec.axis=dup_axis())# not working
GSEA_reports.DF.plot <- GSEA_reports.DF.plot +     theme(axis.text.x=element_text(size=9,angle=90,vjust=-0.001),
...
ggsave2(GSEA.timecourse_soil_all,filename="GSEA.timecourse_soil_all3.png",path=file.path("..","output"),width=20,height=8)
